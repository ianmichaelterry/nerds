...
def _draw_gradient(img: Image.Image, y_start: int, y_end: int,
                   color_top: tuple, color_bot: tuple, alpha: float = 1.0):
    ...
    for y in range(h):
        ...
        a = int(alpha * 255)
        draw.line([(0, y_start + y), (POSTER_W, y_start + y)],
                  fill=(r, g, b))


def _draw_radial_gradient(img: Image.Image, center: tuple, radius: float,
                          color_inner: tuple, color_outer: tuple):
...
def render_poster(bb: Blackboard, output_path: Path) -> Path:
    ...
    palette_node = bb.pick(NERDS.ColorPalette)
    title_node = bb.pick(NERDS.TitleChunks)
    layout_node = bb.pick(NERDS.Layout)
    hero_node = bb.pick(NERDS.HeroImage)
    movie_node = bb.pick(NERDS.MovieData)
    typeface_node = bb.pick(NERDS.Typeface)
    effect_node = bb.pick(NERDS.PostEffect)
    icon_node = bb.pick(NERDS.IconImage)

    # --- Extract values from RDF (with defaults) ---
    key_color = _hex_to_rgb(str(bb.get_property(palette_node, NERDS.keyColor) or "#1e1e28"))
    accent_color = _hex_to_rgb(str(bb.get_property(palette_node, NERDS.accentColor) or "#cc6644"))
    mid_color = _hex_to_rgb(str(bb.get_property(palette_node, NERDS.midColor) or "#755538"))
    ...
    if layout_node:
        for prop, key, default in [
            ...
            layout[key] = float(val) if val and key != "title_align" else (str(val) if val else default)
    else:
        layout = {"title_y": 0.10, "title_align": "center",
    ...
    if icon_node:
        ...
        if icon_b64:
            try:
                ...
                # Size the icon to fit the image area
                icon_target_h = int(float(layout["image_h"]) * POSTER_H * 0.65)
                icon_target_w = icon_target_h  # keep square
                icon_img = icon_img.resize((icon_target_w, icon_target_h),
                                           Image.Resampling.LANCZOS)

                # Position: centered in the image area
                icon_x = (POSTER_W - icon_target_w) // 2
                icon_y = int(float(layout["image_y"]) * POSTER_H) + \
                         (int(float(layout["image_h"]) * POSTER_H) - icon_target_h) // 2
    ...
    if title_node:
        ...
        draw.text((tx + shadow_offset, title_y + shadow_offset), primary,
                  fill=shadow_color, font=font_big)
        ...
        if secondary:
            ...
            if layout["title_align"] == "center":
                ...
                tx2 = 40
            # Slightly lighter accent for subtitle
            sub_color = tuple(min(255, c + 30) for c in accent_color)
            draw.text((tx2, title_y + 58), secondary, fill=sub_color, font=font_small)
    ...
    if movie_node:
        ...
        if director:
            ...
            draw.text((tx, credits_y), dir_text, fill=credit_color, font=font_credits_bold)
        ...
        if actors_str:
            ...
            draw.text((tx, credits_y + 16), cast_text,
                      fill=credit_color, font=font_credits)
        ...
        if year:
            ...
            draw.text((tx, credits_y + 34), year_text,
                      fill=year_color, font=font_year)
    ...
    if icon_node:
        ...
        if term:
            ...
            draw.text(((POSTER_W - tw) // 2, POSTER_H - 16),
                      attr_text, fill=attr_color, font=font_attr)
