...
class Blackboard:
    ...
    def add(
        ...
        self.graph.add(
            (item_node, DCTERMS.created, Literal(self.tick, datatype=XSD.integer))
        )

        # Heat and thermal mass (custom nerds: namespace)
        self.graph.add((item_node, NERDS.heat, HEAT_URIS[heat]))
        self.graph.add(
            (item_node, NERDS.thermalMass, Literal(thermal_mass, datatype=XSD.integer))
        )
    ...
    def query_items(
        self, type_concept: URIRef, min_heat: Heat = Heat.COLD
    ) -> list[URIRef]:
    ...
    def pick(self, type_concept: URIRef, min_heat: Heat = Heat.COLD) -> URIRef | None:
    ...
    def get_recent_critique_issues(
        self, since_tick: int | None = None
    ) -> list[tuple[int, str]]:
        """Get issues from recent failing PosterCritiques.

        Returns list of (tick, issue_string) for critiques that failed (passes=false).
        Optionally only considers critiques since a given tick.
        """
        issues = []
        for item in self.graph.subjects(DCTERMS.type, NERDS.PosterCritique):
            passes = self.graph.value(item, NERDS.passes)
            if passes and str(passes).lower() == "false":
                tick = self.graph.value(item, NERDS.critiqueTick)
                if tick and (since_tick is None or int(tick) > since_tick):
                    issue_str = self.graph.value(item, NERDS.critiqueIssues)
                    summary = self.graph.value(item, NERDS.critiqueSummary)
                    if issue_str:
                        issues.append(
                            (int(tick), str(issue_str), str(summary) if summary else "")
                        )
        return sorted(issues, key=lambda x: x[0], reverse=True)

    def get_structured_issues(self, since_tick: int | None = None) -> list[dict]:
        """Get structured issues from recent failing PosterCritiques.

        Returns list of issue dicts with issue_type, severity, confidence, etc.
        """
        structured_issues = []
        for item in self.graph.subjects(DCTERMS.type, NERDS.PosterCritique):
            passes = self.graph.value(item, NERDS.passes)
            if passes and str(passes).lower() == "false":
                tick = self.graph.value(item, NERDS.critiqueTick)
                if tick and (since_tick is None or int(tick) > since_tick):
                    structured = self.graph.value(item, NERDS.structuredIssues)
                    if structured:
                        try:
                            import json

                            issues = json.loads(str(structured))
                            structured_issues.extend(issues)
                        except:
                            pass
        return structured_issues

    def record_remediation(self, issue_type: str, target: str, success: bool):
        """Record whether a remediation attempt succeeded.

        Tracks what works for different issue types.
        """
        from rdflib import BNode

        remediation = BNode()
        self.graph.add((remediation, RDF.type, NERDS.Remediation))
        self.graph.add((remediation, NERDS.issueType, Literal(issue_type)))
        self.graph.add((remediation, NERDS.remediationTarget, Literal(target)))
        self.graph.add(
            (
                remediation,
                NERDS.remediationSuccess,
                Literal(success, datatype=XSD.boolean),
            )
        )
        self.graph.add(
            (remediation, DCTERMS.created, Literal(self.tick, datatype=XSD.integer))
        )
    ...
    def get_successful_remediations(
        ...
        Returns list of (target, count) for successful remediations.
        If issue_type provided, filter by that type.
        """
        patterns = {}
        for rem in self.graph.subjects(RDF.type, NERDS.Remediation):
            success = self.graph.value(rem, NERDS.remediationSuccess)
            if success and str(success).lower() == "true":
                itype = self.graph.value(rem, NERDS.issueType)
                target = self.graph.value(rem, NERDS.remediationTarget)
                if target and (issue_type is None or str(itype) == issue_type):
                    t = str(target)
