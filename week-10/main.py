...
def _narrate_nerd(nerd_name: str, bb: Blackboard, result_nodes: list) -> str:
    ...
    if nerd_name == "MoviePicker" and result_nodes:
        ...
        details.append(
            f"The MoviePicker queried **Wikidata** via SPARQL for notable films, "
        ...
        details.append("")
        details.append(
            "This data arrived as `schema:Movie`-shaped RDF triples, "
        ...
        if secondary:
            details.append(
                f'Split the title into primary **"{primary}"** and '
        ...
        if str(source) == "omdb":
            details.append("")
            details.append(
                "Keywords were extracted from the OMDb short plot by "
        ...
        details.append(
            f"Selected typeface **{name}** (style: {style}, weight: {weight}). "
        ...
        details.append(
            f"Generated **{count} overlapping color-field blocks** "
        ...
        details.append(
            f"Searched the **Noun Project API** for keyword-derived icons "
        ...
        if effects_list:
            details.append(
                f"Post-processing effects selected: "
                f"{', '.join(f'**{e}**' for e in effects_list)}."
            )
        else:
            details.append(
                "No post-processing effects this time (all coin flips came up tails)."
            )
        ...
        details.append(
            f"Composited two images using **ImageMagick** "
        ...
        details.append(f"Analyzed source visibility in a composite: overall **{pct}**.")
        if scores_str:
        ...
        details.append(
            f"Measured inter-source contrast in a composite: "
        ...
        if issues and issues != "none":
            ...
            details.append(
                f"Completeness: **{pct}%**. Still missing: "
        ...
        score = bb.get_property(node, NERDS.critiqueScore)
        issues = bb.get_property(node, NERDS.critiqueIssues)
        summary = bb.get_property(node, NERDS.critiqueSummary)
        img_path = bb.get_property(node, NERDS.posterImage)
        details.append(f"Rendered a temporary poster")
        if img_path:
            details.append(f"(`{img_path}`) ")
        details.append(
            f"and used the LLM to critique it: **{'passes' if passes else 'fails'}**."
        )
        if score:
            score_val = float(score) if score else 0
            details.append(f"LLM score: **{score_val:.2f}**")
        if summary:
            details.append(f"Summary: {summary}")
        if issues and str(issues):
            details.append(f"Issues noted: {issues}")

    elif nerd_name == "CompletionJudge" and result_nodes:
        details.append(
            "The CompletionJudge checked both the completeness critic (>=80%) "
            "and found a passing LLM poster critique (score >= 0.5), "
            "then **declared the poster complete**."
        )
...
def check_shacl_eligibility(nerd: Nerd, bb: Blackboard, shapes_graph: Graph) -> bool:
    """
...
def select_nerd(
    nerds: list[Nerd], bb: Blackboard, shapes_graph: Graph, verbose: bool = False
) -> Nerd | None:
...
def run(
            ...
            if score >= 0.5:
                ...
                if approved is None:
                    ...
                    existing_score = float(
                        bb.get_property(approved, NERDS.critiqueScore) or 0
                    )
    ...
    if approved:
        ...
        for key, pred in [
            ...
            if val:
                picks[key] = val
        score = bb.get_property(approved, NERDS.critiqueScore)
        print(
            f"Using assets from passing PosterCritique (tick "
            f"{bb.get_property(approved, NERDS.critiqueTick)}, score={score})"
        )
    ...
    print(f"  ({bb.item_count()} items, {len(bb.graph)} triples in the RDF graph)")
