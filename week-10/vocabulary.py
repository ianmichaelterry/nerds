...
def build_concept_scheme() -> Graph:
    ...
    g.add(
        (
            scheme,
            RDFS.comment,
            Literal("Concept scheme for typed items on the NERDS blackboard."),
        )
    )
    ...
    add_concept(
        ...
        "Actor Image",
        NERDS.VisualArtifact,
        "Actor portrait fetched from Wikidata for the movie.",
    )
    add_concept(
        NERDS.WikiMediaImage,
        "WikiMedia Image",
        NERDS.VisualArtifact,
        "Image fetched from Wikimedia Commons based on actor or keyword search.",
    )
    add_concept(
...
def build_shacl_shapes() -> Graph:
    ...
    def _has_type(shape_node, concept_uri, min_count=1, max_count=None):
        ...
        if max_count is not None:
            g.add(
                (prop, SH.qualifiedMaxCount, Literal(max_count, datatype=XSD.integer))
            )
